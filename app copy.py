...
def send_mess(user_number,chat, key,mess_counter_current):
    ...
    c.execute("INSERT INTO chat_table ( user_number,   chat,  [key],  mess_counter    )  VALUES ("+user_number+", '"+chat+"', '"+key+"',   "+mess_counter_current+"    );")
    # rows = c.fetchall()
    # # print(rows)
    # tab=[]
    # for row in rows:
...
def get_mess(user_number,key,mess_counter_current):
    ...
    c.execute("SELECT user_number,chat,key,mess_counter FROM chat_table  where  key='"+key+"' and mess_counter >= "+mess_counter_current+" ;")
    rows = c.fetchall()
    # print(rows)
    tab=[]
    for row in rows:
